...
buildStep() {
    ...
    if [ ! -f "$marker" ]; then
        ...
        echoSection "compile $name"
        "$script" "$@" > "$LOG_DIR/build-${name}.log" 2>&1
        checkStatus $? "build $name"
        touch "$marker"
        echoDurationInSections $START_TIME
    else
        echoSection "skip $name (already built)"
    fi
}
...
buildStep "nasm" "$SCRIPT_DIR/build-nasm.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"

# build custom libiconv version for static linking
# linux has problems with libiconv (glibc)
## TEMPORARY DISABLED: due some problems with zvbi
#if [ "$(uname)" = "Darwin" ]; then
#    buildStep "libiconv" "$SCRIPT_DIR/build-libiconv.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
buildStep "pkg-config" "$SCRIPT_DIR/build-pkg-config.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR"
buildStep "zlib" "$SCRIPT_DIR/build-zlib.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
buildStep "openssl" "$SCRIPT_DIR/build-openssl.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
buildStep "cmake" "$SCRIPT_DIR/build-cmake.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
buildStep "ninja" "$SCRIPT_DIR/build-ninja.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
buildStep "libxml2" "$SCRIPT_DIR/build-libxml2.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
buildStep "fribidi" "$SCRIPT_DIR/build-fribidi.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
buildStep "freetype" "$SCRIPT_DIR/build-freetype.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
FFMPEG_LIB_FLAGS="$FFMPEG_LIB_FLAGS --enable-libfreetype"
buildStep "fontconfig" "$SCRIPT_DIR/build-fontconfig.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
FFMPEG_LIB_FLAGS="$FFMPEG_LIB_FLAGS --enable-fontconfig"
buildStep "harfbuzz" "$SCRIPT_DIR/build-harfbuzz.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
FFMPEG_LIB_FLAGS="$FFMPEG_LIB_FLAGS --enable-libharfbuzz"
echo "NO" > "$LOG_DIR/skip-libharfbuzz"
buildStep "SDL" "$SCRIPT_DIR/build-sdl.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"

if [ $SKIP_LIBBLURAY = "NO" ]; then
    buildStep "libbluray" "$SCRIPT_DIR/build-libbluray.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_SNAPPY = "NO" ]; then
    buildStep "snappy" "$SCRIPT_DIR/build-snappy.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_SRT = "NO" ]; then
    buildStep "srt" "$SCRIPT_DIR/build-srt.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_LIBVMAF = "NO" ]; then
    buildStep "libvmaf" "$SCRIPT_DIR/build-libvmaf.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
buildStep "libass" "$SCRIPT_DIR/build-libass.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
FFMPEG_LIB_FLAGS="$FFMPEG_LIB_FLAGS --enable-libass"

if [ $SKIP_LIBKLVANC = "NO" ]; then
    buildStep "libklvanc" "$SCRIPT_DIR/build-libklvanc.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
buildStep "libogg" "$SCRIPT_DIR/build-libogg.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"

if [ $SKIP_ZIMG = "NO" ]; then
    buildStep "zimg" "$SCRIPT_DIR/build-zimg.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_ZVBI = "NO" ]; then
    buildStep "zvbi" "$SCRIPT_DIR/build-zvbi.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_AOM = "NO" ]; then
    buildStep "aom" "$SCRIPT_DIR/build-aom.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_DAV1D = "NO" ]; then
    buildStep "dav1d" "$SCRIPT_DIR/build-dav1d.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_OPEN_H264 = "NO" ]; then
    buildStep "openh264" "$SCRIPT_DIR/build-openh264.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_OPEN_JPEG = "NO" ]; then
    buildStep "openjpeg" "$SCRIPT_DIR/build-openjpeg.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_RAV1E = "NO" ]; then
    buildStep "rav1e" "$SCRIPT_DIR/build-rav1e.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_SVT_AV1 = "NO" ]; then
    buildStep "svt-av1" "$SCRIPT_DIR/build-svt-av1.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_VPX = "NO" ]; then
    buildStep "vpx" "$SCRIPT_DIR/build-vpx.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_LIBWEBP = "NO" ]; then
    buildStep "libwebp" "$SCRIPT_DIR/build-libwebp.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_X264 = "NO" ]; then
    buildStep "x264" "$SCRIPT_DIR/build-x264.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_X265 = "NO" ]; then
    buildStep "x265" "$SCRIPT_DIR/build-x265.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS" "$SKIP_X265_MULTIBIT"
...
if [ $SKIP_VVENC = "NO" ]; then
    buildStep "vvenc" "$SCRIPT_DIR/build-vvenc.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_VVDEC = "NO" ]; then
    buildStep "vvdec" "$SCRIPT_DIR/build-vvdec.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_LAME = "NO" ]; then
    buildStep "lame" "$SCRIPT_DIR/build-lame.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_OPUS = "NO" ]; then
    buildStep "opus" "$SCRIPT_DIR/build-opus.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_LIBVORBIS = "NO" ]; then
    buildStep "libvorbis" "$SCRIPT_DIR/build-libvorbis.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
if [ $SKIP_LIBTHEORA = "NO" ]; then
    buildStep "libtheora" "$SCRIPT_DIR/build-libtheora.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$CPUS"
...
buildStep "ffmpeg" "$SCRIPT_DIR/build-ffmpeg.sh" "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$OUT_DIR" "$CPUS" "$FFMPEG_SNAPSHOT" "$SKIP_VVDEC" "$FFMPEG_LIB_FLAGS"
